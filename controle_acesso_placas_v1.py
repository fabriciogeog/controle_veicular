...
def yolo_deteccao(frames):
    ...
    #     cv2.rectangle(frames,(x1,y1),(x1+w,y1+h),(255,0,0),2)
    #     cv2.putText(frames, text, (x1,y1-2),cv2.FONT_HERSHEY_COMPLEX, 0.7,(255,0,255),2)
...
# def leitura_string(roi_placa):
#     """Leitura das placas (string)."""
#     placa_lida = pytesseract.image_to_string(roi_placa)
#     placa_lida.strip()
#     placa_lida = placa_lida.replace("-", "")
#     padrao = r"^[A-Z]{3}\d{1}[A-Z0-9]{1}\d{2}$"
#     match = re.match(padrao, placa_lida)
...
if __name__ == "__main__":
    ...
    # url = 'videos_teste/video_teste4.mp4'

    captura = cv2.VideoCapture(url)
    while True:
        quadros = captura.read()[1]
        # quadros = cv2.resize(quadros, [540, 960])
